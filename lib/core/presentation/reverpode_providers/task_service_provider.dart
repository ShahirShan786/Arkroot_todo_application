...
final filteredTasksProvider = Provider<AsyncValue<List<TaskModel>>>((ref) {
  ...
  return tasks.when(
    ...
    error: (error, stackTrace) => AsyncError<List<TaskModel>>(error, stackTrace),
  );
});
          ...
          return filteredTasks.when(
            data: (filteredTaskList) {
              return {
                ...
                'pendingCount': filteredTaskList.where((task) => !task.isCompleted).length,
                'completedCount': filteredTaskList.where((task) => task.isCompleted).length,
